...
router.post('/fetch-insights', async (req, res) => {
  ...
  try {
    ...
    console.log('🌐 Fetching from Graph API:', url);
    ...
    console.error('❌ Error during fetch or insert:', error.response?.data || error.message || error);
    return res.status(500).json({ error: 'Failed to fetch or store Instagram insights' });
  }
});
